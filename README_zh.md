...
🌏 [English](./README.md) · 🖥 [在线页面](https://leifdiao.github.io/doc-atlas/) · 🧩 [看样例面板](https://leifdiao.github.io/doc-atlas/sample-dashboard.html) · ⚖️ [许可](./LICENSE)

---

左侧是统一目录树，右侧是按内容自动选取的模块：执行摘要、首屏关键指标卡、**大尺寸逻辑/关系图（核心卖点，可点击放大）**、数据图表（Chart.js）、冲突对照、章节详情。一切结论都带来源角标，可溯源回「哪个文件第几页」。运行时**先问你要哪种语言的面板**，再开始扫描。

三条核心追求：**① 高度炼化**（看面板就够、不必翻原文）、**② 准确可溯**（零幻觉、不确定就诚实标注、过一道对抗式核查）、**③ 逻辑图是主角**（把核心因果/关系做成醒目大图）。

## 它能做什么

- **多格式归一化** — 任意文档先转成 Markdown 中间层（`content.md` + `assets/` + `meta.json`），保留页码与图片的溯源能力；**PDF 表格按行×列结构化抽取保住数值列**；扫描版 PDF 自动走 OCR 回退。
- **跨文件梳理合并** — 多文件自动去重、识别冲突（数字/日期/结论不一致时显式列出而非静默选一个）、互补整合、判断文件关系，而不是逐文件摘要。
- **高度炼化 + 可信** — 三层阅读模型（概览/要点/细节）、首屏关键指标卡、**炼化体检表**（覆盖率/压缩比/待核实占比）、**对抗式事实核查**（回原件证伪、验算派生数字）。
- **结构化渲染** — AI 只产出结构化 `model.json`，由确定性渲染器编译成单文件 `dashboard.html`（图片 base64 内嵌，仅 Chart.js / Mermaid 走 CDN，离线可开）。
...
0. **定语言 + 扫描确认** — **先问你要哪种语言的面板**，再扫描当前文件夹列出候选文档、与你确认纳入哪些；
1. **归一化** — 每个文件 → `workspace/<name>/{content.md, assets/, meta.json}`（PDF 表格结构化抽取保数值列）；
2. **梳理合并（核心）** — 跨文件去重 / 识冲突 / 互补，重组成 `model.json`，并填炼化体检 + 完整性批判第二遍；
3. **事实核查（对抗式）** — 对外/高风险文档派 subagent 回原件证伪、验算派生数字，修正后再渲染；
4. **渲染** — `model.json (+workspace)` → 单文件 `dashboard.html`；
...
│   ├── normalize.py          阶段一：文档 → content.md/assets/meta.json
...
- **归一化**：`uv` + `python3.11`，首次经你同意装 `markitdown[pdf,docx,pptx,xlsx,xls]` + `pymupdf`（约 290MB，之后复用）；扫描版 PDF 的 OCR 回退可选装 `ocrmypdf`。
- **渲染 / 自检**：系统 `python3` + `playwright`（有 Chrome 时自动回退 `channel="chrome"`）。
