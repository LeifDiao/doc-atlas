...
1. **日期更新者优先**：`meta.json` 的 date、文件名里的版本号/日期、正文里的“截至 X”。新版本默认覆盖旧版本（除非旧版是有意保留的基线）。
...
- SourceRef 至少给 `page`（有页码时）或 `loc`（无页码时：`§2.1` / `Slide 4` / `Sheet1!B2`）之一。页码/定位从对应 `meta.json` 的 heading_index / 页码映射里取，不要凭空写。
- `file_id` 必须命中某个 `files[].id`。
- 合并节点要列**全部**出处，不是只留一个。
...
### 9.3 填 `distillation_report` 并守阈值

写完即填顶层 `distillation_report`（字段见 `model-schema.md` §3.1c），并据此自检、交付时讲给用户：

- `section_coverage` **必须 = 100%**（每个源章节至少映射到一个 outline 节点），缺章回去补。
- `todo_ratio > 10%` → **先别交付**：极可能是表格数值列没抽好，按 §6.1 回查原件补回，能恢复的不再标“待核实”。
- `claims_with_source < 100%` → 有无源声明，补源或删。
- `compression_ratio` 落在 ~3:1 到 ~10:1 才算“既炼又不漏”；接近 1:1 = 只是搬运，过高（如 >15:1）= 可能漏。
